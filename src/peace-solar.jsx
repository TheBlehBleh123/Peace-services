...
/*
  ╔══════════════════════════════════════════════════════════════╗
  ║  IMAGE SWAP GUIDE — replace Unsplash URLs for production    ║
  ╚══════════════════════════════════════════════════════════════╝
...
/*
  ...
  ║  3. Set both values below                                    ║
  ║  For production: call API server-side to protect your key    ║
  ╚══════════════════════════════════════════════════════════════╝
*/
const GOOGLE_CONFIG = {
  apiKey: "",
...
const Logo = ({ height = 80 }) => (
  <img src={IMG.logo} alt="Peace Solar & Window Cleaning" referrerPolicy="no-referrer" style={{ height, width: "auto" }} />
);
...
const GlobalStyles = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .svc-card img { transition: transform .8s cubic-bezier(.22,1,.36,1); }
    ...
    @media(max-width:768px){
      ...
      .hero-img-wrap{height:60vh;border-radius:20px!important}
...
const Grain = () => (
  <div style={{
    position:"fixed",inset:0,pointerEvents:"none",zIndex:9999,opacity:.022,
...
const useGoogleReviews = () => {
  ...
  useEffect(() => {
    ...
    // Production: fetch('/api/reviews').then(r => r.json()).then(data => {
...
const Navbar = () => {
  ...
  return (
    <nav style={{
      ...
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",maxWidth:1400,margin:"0 auto",padding:"0 20px" }}>
        <a href="#" style={{lineHeight:0}}><Logo height={scrolled?56:72}/></a>
      ...
      {open&&(
        <div className="mob-only" style={{background:C.cream,borderBottom:`1px solid ${C.navy}15`,padding:"24px 28px",display:"flex",flexDirection:"column",gap:20}}>
          ...
          <a href="tel:760299" onClick={()=>setOpen(false)} style={{fontFamily:fontDisplay,fontSize:13,fontWeight:700,color:C.sage,textDecoration:"none",display:"flex",alignItems:"center",gap:8}}>
...
const ReviewCard = ({ review }) => (
  <div style={{
    ...
    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
      <div style={{display:"flex",alignItems:"center",gap:12}}>
        <div style={{
          ...
          {!review.profile_photo_url && review.author_name[0]}
        </div>
        <div>
    ...
    <div style={{display:"flex",gap:2}}>
      {[...Array(5)].map((_,i)=>(
        <Star key={i} size={14} fill={i < review.rating ? "#FBBC05" : "#ddd"} color={i < review.rating ? "#FBBC05" : "#ddd"}/>
      ))}
    </div>
    <p style={{fontFamily:fontSans, fontSize:14, color:`${C.navy}cc`, lineHeight:1.7, flexGrow:1}}>
      "{review.text}"
    </p>
  </div>
);
...
const ReviewMarquee = ({reviews, reverse=false}) => (
  <div style={{overflow:"hidden",padding:"12px 0"}}>
    <div style={{display:"flex",gap:20,width:"max-content",animation:`${reverse?"marquee-right":"marquee-left"} 50s linear infinite`}}>
...
export default function App() {
  ...
  return (
    <div style={{minHeight:"100vh",background:C.cream,color:C.navy,fontFamily:fontSans}}>
      ...
      {/* ═══ 1 · HERO TEXT ═══ */}
      <section className="sec-hero-text" style={{paddingTop:110,paddingBottom:8,textAlign:"center"}}>
        <div className="hero-anim" style={{maxWidth:920,margin:"0 auto",padding:"0 24px"}}>
          <span className="hero-anim-d1" style={{fontFamily:fontDisplay,fontWeight:700,letterSpacing:".3em",textTransform:"uppercase",fontSize:11,color:C.sage,display:"block",marginBottom:20}}>
          ...
          <p className="hero-sub" style={{fontFamily:fontSans,fontWeight:300,color:`${C.navy}aa`,maxWidth:620,margin:"0 auto",lineHeight:1.8,marginBottom:40}}>
        ...
        <div className="hero-anim-d2" style={{display:"flex",justifyContent:"center",padding:"0 24px",marginBottom:32}}>
          <a href="tel:760299" className="cta-main" style={{
      ...
      {/* ═══ 2 · HERO IMAGE — parallax only, NO opacity fade ═══ */}
      <section ref={heroRef} style={{position:"relative",overflow:"hidden",padding:"0 16px"}}>
        <div style={{transform:`translateY(${heroOffset}px)`}}>
          <div className="hero-img-wrap" style={{position:"relative",width:"100%",borderRadius:28,overflow:"hidden"}}>
            <img src={IMG.hero} alt="Peace Solar & Window Cleaning" referrerPolicy="no-referrer"
            ...
            <div className="hero-bottom" style={{
              ...
              <div style={{maxWidth:500}}>
                <h2 className="hero-btm-h2" style={{
                  fontFamily:fontSans, fontWeight:700, color:C.cream, marginBottom:8,
              ...
              <div className="stats-row" style={{display:"flex",gap:14}}>
                {[{n:"400+",l:"Families Served"},{n:"100+",l:"5-Star Reviews"}].map(s=>(
                  <div key={s.l} className="stat-box" style={{
                    ...
                    <div className="stat-num" style={{fontFamily:fontSerif,fontSize:38,fontWeight:700,color:"#fff",textShadow:"0 2px 12px rgba(0,0,0,1)",lineHeight:1.1}}>{s.n}</div>
                    <div className="stat-lbl" style={{fontFamily:fontDisplay,fontSize:10,textTransform:"uppercase",letterSpacing:".22em",fontWeight:800,color:"#fff",marginTop:4}}>{s.l}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      {/* ═══ 3 · VALUES ═══ */}
      <section style={{padding:"80px 24px",background:`${C.offwhite}80`}}>
            ...
            {[
              ...
              <FadeIn key={i} delay={i*.15}>
                <div style={{display:"flex",flexDirection:"column",alignItems:"center",textAlign:"center"}}>
                  <div style={{width:72,height:72,borderRadius:"50%",background:`${C.sage}15`,display:"flex",alignItems:"center",justifyContent:"center",marginBottom:24}}>{v.icon}</div>
                  <h3 style={{fontFamily:fontSerif,fontSize:26,fontWeight:400,marginBottom:14,letterSpacing:"-.01em"}}>{v.t}</h3>
                  <p style={{fontFamily:fontSans,fontSize:14,color:`${C.navy}99`,lineHeight:1.8,maxWidth:320}}>{v.d}</p>
                </div>
              </FadeIn>
            ))}
          </div>
      ...
      {/* ═══ 4 · PACKAGES — thick borders, navy on first two, sage on Pro ═══ */}
        ...
        <div style={{maxWidth:1200,margin:"0 auto"}}>
          <FadeIn>
            <div style={{textAlign:"center",marginBottom:64}}>
              ...
              <p style={{fontFamily:fontSans,opacity:.7,fontSize:18,fontWeight:300}}>Never worry about keeping up on cleanings again.</p>
            </div>
          </FadeIn>
          <div className="price-grid" style={{marginBottom:56}}>
            {[
              {name:"One-Time Cleaning",features:[{t:"Full Price",on:false},{t:"Auto-Scheduled",on:false},{t:"Free Screen-Washing",on:false},{t:"Unlimited Rain-Insurance",on:false}],hl:false,borderColor:`#000000`},
              {name:"Bi-Annual Basic Package",features:[{t:"10% OFF Each Clean",on:true},{t:"Scheduled 2x a year",on:true},{t:"Free Screen-Washing",on:false},{t:"Unlimited Rain-Insurance",on:false}],hl:false,borderColor:`#000000`},
              {name:"Quarterly Pro Package",features:[{t:"20% OFF Each Clean",on:true},{t:"Scheduled 4x a year",on:true},{t:"Free Screen-Washing",on:true},{t:"Unlimited Rain-Insurance",on:true}],hl:true,borderColor:C.sage},
            ].map((plan,pi)=>(
              <FadeIn key={pi} delay={pi*.12}>
                <div style={{
                  background: C.cream,
                  color: C.navy,
                  borderRadius: 28,
          ...
          <div style={{textAlign:"center"}}>
            <a href="tel:760299" style={{
              ...
              boxShadow:`0 30px 60px ${C.sage}44`,borderBottom:`4px solid ${C.navy}15`,transition:"all .4s",
            }}
          ...
          <FadeIn>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:56,flexWrap:"wrap",gap:24}}>
              <div style={{maxWidth:520}}>
                ...
                <p style={{fontFamily:fontSans,color:`${C.navy}99`,fontSize:16,lineHeight:1.7}}>From the desert sun to the winter lights, we keep your home shining year-round.</p>
              </div>
      ...
      {/* ═══ 6 · GOOGLE REVIEWS — no "leave a review" button ═══ */}
      <section style={{padding:"80px 0",overflow:"hidden"}}>
        <FadeIn>
          <div style={{textAlign:"center",marginBottom:48,padding:"0 24px"}}>
            <div style={{display:"flex",justifyContent:"center",alignItems:"center",gap:10,marginBottom:16}}>
              <GoogleG size={28}/>
              <h2 style={{fontFamily:fontSerif,fontSize:"clamp(28px,4vw,44px)",fontWeight:400,letterSpacing:"-.02em",margin:0}}>
                Google Reviews
              </h2>
            </div>
        ...
        <div className="story-grid" style={{maxWidth:1200,margin:"0 auto"}}>
          ...
          <FadeIn delay={.2}>
            <div style={{display:"flex",flexDirection:"column",justifyContent:"center"}}>
              ...
              <p style={{fontFamily:fontSans,fontSize:18,color:`${C.navy}bb`,lineHeight:1.9,marginBottom:24,fontWeight:300}}>
              ...
              <p style={{fontFamily:fontSans,fontSize:18,color:`${C.navy}bb`,lineHeight:1.9,fontWeight:300}}>
      ...
      {/* ═══ FOOTER ═══ */}
      <footer style={{padding:"48px 24px",borderTop:`1px solid ${C.navy}12`}}>
        <div className="footer-row" style={{maxWidth:1200,margin:"0 auto",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:24}}>
          <a href="#" style={{lineHeight:0}}><Logo height={56}/></a>
