...
const BASE_ID = () => process.env.AIRTABLE_BASE_ID || 'apptscqYbVav0mpLi';
const TABLE = () => process.env.AIRTABLE_TABLE || 'tbl30v5JABh4EeE74';
const TOKEN = () => (process.env.AIRTABLE_TOKEN || '').trim();
const VERIFY_SECRET = () => process.env.VERIFY_SECRET || '';

// Canonical email used for identity/dedup. Lowercase + trim + NFC, and for
// Gmail collapse the alias tricks (dots and "+tags" all reach the same inbox),
// so foo+1@gmail.com / f.o.o@gmail.com can't masquerade as separate people.
export function normalizeEmail(email) {
  let e = String(email || '').trim().toLowerCase();
  try { e = e.normalize('NFC'); } catch { /* ignore */ }
...
// email -> short stable alphanumeric code (52 bits of the hash, base36).
export function referralCode(email) {
  const h = crypto.createHash('sha256').update(normalizeEmail(email)).digest('hex');
  return parseInt(h.slice(0, 13), 16).toString(36).toUpperCase();
}

// email -> unforgeable verification token (HMAC, first 32 hex chars).
// Hard-fail if the secret is missing, so a misconfig can't turn this into a
// public function that anyone could forge tokens with.
export function verifyToken(email) {
  const secret = VERIFY_SECRET();
  if (!secret) { const e = new Error('VERIFY_SECRET is not configured'); e.code = 'NOT_SETUP'; throw e; }
...
// Look up a person's row by their (server-derived, always [0-9A-Z]) referral
// code — never by raw email — so no user input ever reaches an Airtable
// formula. This also makes apostrophe/quote emails work instead of 502-ing.
export async function findByCode(code) {
  const formula = `{Referral Code}='${esc(code)}'`;
  const data = await airtable('GET', `?maxRecords=1&filterByFormula=${encodeURIComponent(formula)}`);
  return (data.records && data.records[0]) || null;
}

// Count only CONFIRMED referrals — an unverified referral isn't a real entry.
export async function countReferrals(code) {
  const formula = `AND({Referred By}='${esc(code)}', {Verified})`;
  let count = 0;
  let offset;
  do {
